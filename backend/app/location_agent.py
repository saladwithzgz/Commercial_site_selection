...
class LocationAnalysisAgent:
    ...
    def extract_addresses(self, user_input: str) -> List[str]:
        """从用户输入中提取候选地址"""
        # 常见地址关键词
        patterns = [
    ...
    def extract_store_type(self, user_input: str) -> str:
        """提取店铺类型"""
    ...
    def extract_brand_positioning(self, user_input: str) -> Optional[str]:
        """提取品牌定位"""
        positioning_map = {
            "高端": "高端精品",
            "平价": "学生平价",
    ...
    async def analyze(self, user_input: str) -> Dict[str, Any]:
        ...
        # 第一阶段：信息提取
        addresses = self.extract_addresses(user_input)
        store_type = self.extract_store_type(user_input)
        brand_positioning = self.extract_brand_positioning(user_input)

...
class AmapAPI:
    ...
    def __init__(self):
        ...
        self.available = True
        if self.key and self.key != "your_amap_api_key_here":
            logger.info(f"高德地图API客户端初始化完成，已配置API Key")
        else:
            logger.info("高德地图API客户端初始化完成，未配置API Key（使用模拟数据）")
    ...
    def _load_api_key(self) -> str:
        ...
        try:
            ...
            import os
            load_dotenv()
            return os.getenv("AMAP_API_KEY", "your_amap_api_key_here")
        except Exception:
    ...
    async def geocode(self, address: str, city: str = "北京") -> Optional[Dict[str, float]]:
        """地理编码"""
        try:
            logger.info(f"调用高德地理编码: {address}")
            ...
            logger.error(f"高德地理编码调用失败: {e}")
            return None
    
    async def around_search(self, location: str, keywords: str, radius: int = 500) -> List[Dict]:
        """周边搜索"""
        try:
            logger.info(f"调用高德周边搜索: location={location}, keywords={keywords}, radius={radius}")
            ...
            if result.get("status") == "1":
                ...
                logger.info(f"周边搜索成功: {len(pois)} 个结果")
                return pois
            return []
        except Exception as e:
            logger.error(f"高德周边搜索调用失败: {e}")
            return []
    
    async def text_search(self, keywords: str, city: str = "北京", types: str = "") -> List[Dict]:
        """关键词搜索"""
        try:
            logger.info(f"调用高德关键词搜索: keywords={keywords}, city={city}")
            ...
            if result.get("status") == "1":
                pois = result.get("pois", [])
                logger.info(f"关键词搜索成功: {len(pois)} 个结果")
                return pois
            return []
        except Exception as e:
            logger.error(f"高德关键词搜索调用失败: {e}")
            return []
...
class MCPServiceManager:
    ...
    async def geocode(self, address: str, city: str = "北京") -> Tuple[Optional[Dict[str, float]], str]:
        """地理编码（带容错）"""
        # 尝试使用高德API（如果配置了key）
        if self.amap_api.key and self.amap_api.key != "your_amap_api_key_here":
            result = await self.amap_api.geocode(address, city)
            if result:
                return result, AmapAPI.SERVICE_NAME
    ...
    async def _collect_competitor_data(self, location_data: LocationData):
        ...
        # 尝试使用高德API
        if self.amap_api.key and self.amap_api.key != "your_amap_api_key_here":
            pois = await self.amap_api.around_search(
                location=location_str,
                keywords="奶茶店",
                radius=1000
            )
            if pois:
                location_data.competitor_count = len(pois)
                location_data.competitor_list = [poi.get("name", "") for poi in pois[:10] if poi.get("name")]
                logger.info(f"竞品数据: {location_data.address} -> {location_data.competitor_count}家")
                return
    ...
    async def _collect_customer_flow_data(self, location_data: LocationData):
        ...
        # 尝试使用高德API
        if self.amap_api.key and self.amap_api.key != "your_amap_api_key_here":
            office_pois = await self.amap_api.around_search(
                location=location_str,
                keywords="写字楼",
                radius=1000
            )
            ...
            school_pois = await self.amap_api.around_search(
                location=location_str,
                keywords="学校",
                radius=2000
            )
            ...
            mall_pois = await self.amap_api.around_search(
                location=location_str,
                keywords="商场",
                radius=1000
            )
    ...
    async def _collect_facilities_data(self, location_data: LocationData):
        ...
        # 尝试使用高德API
        if self.amap_api.key and self.amap_api.key != "your_amap_api_key_here":
            conv_pois = await self.amap_api.around_search(
                location=location_str,
                keywords="便利店",
                radius=500
            )
            ...
            food_pois = await self.amap_api.around_search(
                location=location_str,
                keywords="餐饮",
                radius=500
            )
